...
- Only valid while the loan's `Status = Pending Board Approval` and the caller's own `Approvals` entry is still `Pending`. Voting again after already voting, or voting on a loan that's no longer pending, is rejected (400 — a state conflict on otherwise-valid input, not a body-validation error, so it follows this codebase's established 400-for-business-rule-violation convention rather than 422) — **a cast vote is final.**
